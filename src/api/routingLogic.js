...
async function setEndpointOnMessage(senderPSID, path) {
	...
	switch (route.status) {
		case "action":
			...
			if (route.requiresMeta) {
				...
				// Need to get senderPSID
				callSendApi(senderPSID, { text: `Please enter ${userMessageTable.metaTypeExpected}` });
...
function handleMetadata(senderPSID, userMetaMessage) {
	const data = { meta: processMetadata(userMetaMessage, userMessageTable.metaTypeExpected) };
