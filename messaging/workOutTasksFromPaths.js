...
async function workOutTasksFromPaths(senderPSID, path) {
	let response;

	if (path) {
		const val = await pathDb.findOne({ pathId: path.toLowerCase() });
		if (val) {
			routingLogic.setEndpointOnMessage(senderPSID, path);
			if (val.type === "integer") {
